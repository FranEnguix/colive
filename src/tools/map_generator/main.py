...
def run(cord1,cord2):
    def girar_derecha():
        global labelRotate, imagenRotate, name2,angulo, ventana2b, fotoRotate
        def cerrar():
            ventana3b.destroy()
        ...
        def guardar():
            ...
            print("Name2 ", name2)
            im3 = im3.rotate(angulo, expand=1)
            #im3.show()
            if (im3.save(name2)):
                print("Imagen salvada y redimensionada")
                
        angulo+=5
        print("Prueba ", angulo)
        ventana3b = tk.Toplevel()
        canvas3b = tk.Canvas(ventana3b, width=600, height=400)
        canvas3b.pack()
        ventana3b.title("Rotación de la imagen")
        ventana3b.geometry("500x600")
 #       imagenColor = Image.open(name)
        im2 = Image.open(name2)
        im2 = reducir_imagen(im2)
        im2 =  im2.rotate(angulo)
 #       im2.show()
        fotoRotate2 = ImageTk.PhotoImage(im2)
#         print("A")
#         print(fotoRotate2)
        TT = tk.Label(ventana3b, image=fotoRotate2)
        TT.image=fotoRotate2
#        print("B")
        TT.place(x=50,y=100)
#         print(TT)
        bbb=tk.Button(ventana3b, text="Cerrar", command=cerrar)
        bbb.place(x=90,y=400)
        bb2=tk.Button(ventana3b, text="Guardar", command=guardar)
        bb2.place(x=180,y=400)
...
#         print(TT)
    ...
    if(cv2.imwrite(name, img)):
        ...
        name2=name
        ventana2b = tk.Tk()
        canvas2b = tk.Canvas(ventana2b, width=800, height=300)
        canvas2b.pack()
#        print("Previo")
        ventana2b.title("Previsualización de la imagen")
        ventana2b.geometry("500x300")
        imagenColor = Image.open(name)
        #rotateImage= imagenColor.rotate(45)
        imagenColor = reducir_imagen(imagenColor)
#         imagenRotate = imagenColor.copy()
#         imagenRotate = imagenRotate.rotate(angulo)
        fotoColor = ImageTk.PhotoImage(imagenColor)
#         fotoRotate= ImageTk.PhotoImage(imagenRotate)
        labelColor = tk.Label(ventana2b, image=fotoColor)
#         labelRotate = tk.Label(ventana2b, image=fotoRotate)
        
        labelColor.place(x=50,y=100)
#         labelRotate.place(x=50,y=400)
        boton_Recorte=tk.Button(ventana2b, text="Recortar", command=lambda: Mod_crop.recortar(name) )
        boton_Recorte.place(x=250,y=30)
        boton_GiroDer= tk.Button(ventana2b, text="Giro derecha 5º", command=girar_derecha)
        boton_GiroDer.place(x=20,y=30)
        boton_GiroIzq= tk.Button(ventana2b, text="Giro izquierda 5º", command=girar_izquierda)
        boton_GiroIzq.place(x=150,y=30)
        
#         print("Antes de mainloop")
        ventana2b.mainloop()
#        print("Despues de mainloop")
...
if os.path.isfile(prefs_path):
    ...
    # script_file_path = "map_script.js"
    # with open(script_file_path, "r") as script_file:
    #     script_content = script_file.read()
    # m.get_root().html.add_child(folium.JavascriptLink('./map_script.js'))
    # folium.MacroElement(element_type='script', code=script_content).add_to(m)
