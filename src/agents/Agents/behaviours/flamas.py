...
class EntityShell:
    ...
    # FLAMAS
    async def flamas_setup(agent: Agent):
        if agent.name == "agente1":
            agent.flamas_role = "server"
        else:
            agent.flamas_role = "client"
            agent.flamas_server = "agente1@localhost"

        if agent.name == "agente2":
            agent.flamas_weights = [0] * 10 
        elif agent.name == "agente3":
            agent.flamas_weights = [10] * 10 

    async def flamas_receive(agent: Agent):
        time.sleep(1)

    async def flamas_train(agent: Agent):
        time.sleep(1)

    async def flamas_send(agent: Agent):
        time.sleep(1)
